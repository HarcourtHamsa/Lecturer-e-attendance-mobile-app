...
import { getAttendanceList } from "../../utils";
...
function Item({ time, data }) {
  ...
  return (
    <>
      ...
      {data.map((d, i) => {
        return (
          <View className="h-14 items-center mb-2 flex flex-row justify-between" key={i}>
...
export default function Attendance({ route, navigation }) {
  ...
  useEffect(() => {
    async function fetchData() {
      const res = await getAttendanceList(courseId);
      console.log(res);
  ...
  return (
    <View className="flex-1">
      ...
      <View className="flex-1 p-4">
        <Text className="text-2xl">Attendance List</Text>
        {/* <CustomSearchBar placeholder="Search attendance list"/> */}
